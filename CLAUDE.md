...
**When the bake stops on a missing session**, do not simply acknowledge it. The audit already re-fetches every suspect direct from Yahoo with the CORS proxies disabled, because a degraded fetch and a real gap look identical in the data — the first CI run of this guard flagged eight US ETFs whose bars were present and healthy moments later. If a gap survives that, verify it by hand against the intraday series (`range=5d&interval=5m` recovers the session and its volume), then record it in `data_gaps.json` with what you found. Acknowledging is a deliberate act that leaves a reviewable record; an entry there is read by later sessions as verified fact, so a careless one is worse than the stop it clears. Note that the 5-minute series ends before the SGX closing auction, so a recovered close carries roughly 0.2 per cent error — good enough to confirm a session happened, not good enough to publish as a close.

Acknowledged gaps are published in `meta.json` and consumed by `_hasDayWindowGap` in `template.html`. A gapped name is withdrawn from the Intraday and 1-Day cards on both sides — numerator and denominator — rather than assumed flat, and the cards carry the exclusion count in amber with the positions named in the tooltip. Attribution routes through the same predicate. Longer windows are untouched.
